...
void RapbitPro()
{
  ...
  display.drawString(15, 0, "ETC");
  display.drawString(15, 30, "ROBOT");
  display.display();
  delay(700);
  display.clear();
  display.display();
...
int ReadLightSensor(int analog_CH)
{
  ...
  if (line_color == 0)
    value = map(analogRead(sensor_pin[analog_CH]), min_sensor_values[analog_CH], max_sensor_values[analog_CH], 100, 0);
  else if (line_color == 1)
    value = map(analogRead(sensor_pin[analog_CH]), min_sensor_values[analog_CH], max_sensor_values[analog_CH], 0, 100);
  if (value < 0)
    value = 0;
  else if (value > 100)
    value = 100;
...
void turnUntill(int stopAngle)
{
  int startAngle = 0;
  startAngle = Read_angle(2); // 997
  stopAngle *= -1;
  stopAngle += startAngle;    // -90 + 997 = 907
  if (stopAngle < startAngle) // 907 < 997
  {
...
void run_straight(int _time, int _speed)
{
  int startAngle = 0;
  startAngle = Read_angle(2);
  for (int i = 0; i < _time; i++)
  {
    ...
    if (Read_angle(2) < startAngle)
    {
      motor(1, 1, 20);
      motor(2, 1, _speed);
    }
    else if (Read_angle(2) > startAngle)
    {
      motor(1, 1, _speed);
      motor(2, 1, 20);
    }
...
void run_back(int _time, int _speed)
{
  int startAngle = 0;
  startAngle = Read_angle(2);
  for (int i = 0; i < _time; i++)
...
void averrage_yellow()
{
  int aYY1 = 0;
  int aYY2 = 0;
  int aYY3 = 0;
  for (int aYY = 1; aYY <= 5; aYY++)
  {
    while ((digitalRead(23)) == 1)
    {
      display.clear();
      ...
      display.setFont(ArialMT_Plain_10);
      display.drawString(0, 0, String(String("put sensor 3 4 5 on yellow")));

      display.setFont(ArialMT_Plain_10);
      display.drawString(0, 10, String(String("then press sw1 button")));

      display.setFont(ArialMT_Plain_10);
      display.drawString(0, 20,
                         String(((String("L2: ") + String((analog(33)))))));

      display.setFont(ArialMT_Plain_10);
      display.drawString(0, 30,
                         String(((String("L3: ") + String((analog(32)))))));

      display.setFont(ArialMT_Plain_10);
      display.drawString(0, 40,
                         String(((String("L4: ") + String((analog(35)))))));
      display.display();
      delay(10);
    }
  ...
  while (1)
  {
    display.clear();
    display.setFont(ArialMT_Plain_10);
    display.drawString(0, 0,String(((String("L2 : ") + String(analog(33))))));
    display.drawString(0, 10,String(((String("L3 : ") + String(analog(32))))));
    display.drawString(0, 20,String(((String("L4 : ") + String(analog(35))))));
    display.display();
    delay(10);
  }
}
void Run_CheckPoint(int aS1,int aS2,int aS3)
{
  uint16_t setpoint;
  float present_position;
  float errors = 0;
  ...
  int min_aS3 = aS3 - 50;
  int max_aS3 = aS3 + 50;
  while ((as2 < min_aS2 || as2 > max_aS2) || ((as1 < min_aS1 || as1 > max_aS1)&&(as3 < min_aS3 || as3 > max_aS3)))
  {
    present_position = readline();
    setpoint = 200.0;
    errors = setpoint - present_position;
    derivative = (errors - previous_error);
    output = 0.5 * errors + 1 * derivative;
    int max_output = 100;
    if (output > max_output)
      output = max_output;
    else if (output < -max_output)
